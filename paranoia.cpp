...
// Then converted to Pascal by B. A. Wichmann.
// David M. Gay and Thos Sumner converted the Pascal version to C.
// Richard Karpinski polished it

// This is a truncated C++ rewrite of PARANOIA for basic TinyFloat testing
...
int main() {
    ...
    std::cout << "Program is now RUNNING tests on small integers:\n";
    ...
    std::cout << "Searching for Radix and Precision.\n";
    FLOAT W = One;
    FLOAT Y;
    do {
        W = W + W;
    ...
    FLOAT U1 = One / W;
    FLOAT U2 = Radix * U1;
    std::cout <<  "Closest relative separation found is U1 = " << U1 << "\nRecalculating radix and precision\n";
...
//  FLOAT E3 = Precision;
    ...
    if ( U1 == E1 ) {
        ...
        std:: cout << "gets better closest relative separation U1 = " << U1 << ".\n";
    }
    ...
    if ( Radix == E0 ) {
        ...
        std:: cout << "MYSTERY: recalculated Radix = " << Radix << ".\n";
    }
    ...
    if ((Precision != (FLOAT)floorf(Precision)) || (Radix == One)) {
        ...
        std::cout << "of significant digits but, by itself, this is a minor flaw.\n";
    }

    if (Radix == One) {
        std::cout << "logarithmic encoding has precision characterized solely by U1.\n";
    } else {
        std::cout << "The number of significant digits of the Radix is "<< Precision << ".\n";
    }
    ...
    if ((X1 != Y1) || (X1 != Z1)) {
        ...
        std::cout << "respectively " << X1 << ", " << Y1 << ", " << Z1 << "\n";
        std::cout << "are symptoms of inconsistencies introduced\n";
        std::cout << "by extra-precise evaluation of arithmetic subexpressions.\n";
        ...
        if ((Z1 != U1) || (Z2 != U2)) {
            if ((Z1 >= U1) || (Z2 >= U2)) {
                ...
                if ((Z1 <= Zero) || (Z2 <= Zero)) {
                    ...
                    std::cout << ", or exact rational arithmetic a result\n";
                    std::cout << "Z1 = " << Z1 << ", or Z2 = " << Z2;
                    //        notify("of an\nextra-precision");
                }
                if (Z1 != Z2 || Z1 > Zero) {
                    X = Z1 / U1;
                    ...
                    std::cout << "precisely with about " << (Q / (FLOAT)logf(Radix)) << " extra B-digits, i.e.\n";
                    std::cout << "roughly " << (Q / (FLOAT)logf(10.)) << " extra significant decimals.\n";
                }
                std::cout << "That feature is not tested further by this program.\n";
            }
        }
    ...
    Z = Radix * U2;
    X = One + Z;
    Y = (FLOAT)fabsf((X + Z) - X * X) - U2;
    X = One - U2;
    Z = (FLOAT)fabsf((X - U2) - X * X) - U1;
    ...
    if (F9 != One && F9 - One >= Zero) {
        ...
        std::cout << "  subtraction yields  (1-U1) - 1 = 0 , thereby vitiating\n";
        std::cout << "  such precautions against division by zero as\n";
        std::cout << "  ...  if (X == 1.0) {.....} else {.../(X-1.0)...}\n";
    }
    if (GMult == Yes && GDiv == Yes && GAddSub == Yes) std::cout <<  "     *, /, and - appear to have guard digits, as they should.\n";

    Rounding RMult, RDiv, RAddSub;
    FLOAT RadixD2, A1, AInvrse, Y2, S;
    bool Done;
    ...
    do {
        ...
        do {
            X = AInvrse;
            AInvrse = AInvrse / A1;
        } while ( ! ((FLOAT)floorf(AInvrse) != AInvrse));
        Done = (X == One) || (A1 > Three);
        if (! Done) A1 = Nine + One;
    } while ( ! (Done));
    if (X == One) A1 = Radix;
    ...
    } else std::cout << "* is neither chopped nor correctly rounded.\n";
    ...
    if (RDiv == Other) std::cout << "/ is neither chopped nor correctly rounded.\n";
    BInvrse = One / Radix;
    ...
    if (GAddSub == Yes)
    {
        ...
        if ((X == Zero) && (Y == Zero))
        {
            ...
            else std::cout << "Addition/Subtraction neither rounds nor chops.\n";
        }
        else std::cout << "Addition/Subtraction neither rounds nor chops.\n";
    }
    else std::cout << "Addition/Subtraction neither rounds nor chops.\n";
    ...
    if ((GMult == Yes) && (GDiv == Yes) && (GAddSub == Yes)
            ...
            && (RAddSub == Rounded) && ((FLOAT)floorf(RadixD2) == RadixD2)) {
